...
def load(
    ...
    """
    test_ds, train_ds = _loadFromDirectory(
        train_dir, test_dir, OneChannelOnly, image_size, batch_size
    )
    ...
    if augmentations is not None:
        assert isinstance(
            augmentations, int
        ), "The number of augmentations must be an integer"

        augmented_datasets = train_ds.flat_map(
            lambda img, label: _augment_multiple(
                img, label, augmentations, OneChannelOnly
            )
        )
